...
Add provider accounts to CPA, define their concrete IDs and logical routes in private `aisix/resources.yaml`, then create an OpenAI-compatible upstream account in Sub2API. Keep every direct AISIX `display_name` byte-identical to its CPA `model_name`, including `/`; only client-facing logical combos use separate bare names. Every logical route uses the same bounded fallback status set `[400, 401, 402, 403, 404]`: `400` covers observed disabled/unavailable targets, `401` invalid or revoked target credentials, `402` unavailable payment/credit/balance/quota so later targets remain reachable, `403` denied accounts/models, and `404` absent or unexposed targets. `retries` and `max_fallbacks` remain the attempt bounds; including `400` or `402` can retry malformed or billing-failed requests and therefore increases load.
...
Virtual DNS preserves the requested hostname for Squid CONNECT while preventing client-side DNS escape. Do not bind a host file over the tunnel owner's `/etc/resolv.conf`. The tunnel intentionally keeps only its disposable container layer writable because tun2proxy must rewrite Docker's runtime resolver file and clean up TUN state during startup and teardown; it has no persistent writable mount, remains unprivileged, and receives only `NET_ADMIN` and `DAC_OVERRIDE`. The `DAC_OVERRIDE` capability is required because Docker re-mounts `/etc/resolv.conf` read-only on container restart/boot-restore, while a fresh create leaves it writable: without it, tun2proxy's fallback direct write fails with EACCES and the tunnel crash-loops until recreated. CPA reaches provider-sidecar only through `cpa-provider-sidecar-relay`; the tunnel reaches Squid only through `provider-sidecar-squid-relay`. Each direction has separate two-member source and target networks, and a failed relay or tunnel loses connectivity instead of gaining direct Internet access.

For recovery, treat CPA, `cpa-provider-sidecar-relay`, provider-sidecar tunnel, and provider-sidecar as one unit. A missing/expired/mismatched TLS file must keep path failed closed; repair initializer state, validate, then recreate coupled services. The tunnel runs `restart: unless-stopped` with a `tun2proxy-bin --version` healthcheck so `depends_on: service_healthy` can gate the sidecar on a ready netns (boot-time start race). The fail-closed property is unchanged: if the tunnel is down, the sidecar has no route anywhere. A provider-sidecar process keeps the shared network namespace alive after the tunnel owner exits, so a tunnel restart leaves the sidecar in a stale netns until the pair is rebuilt. Rebuild the pair in order:
...
AISIX owns HTTP/SSE alias routing over concrete CPA targets. Sub2API owns WebSocket ingress and reconstructs explicit full-history requests through `http_bridge`; the internal models gateway passes authenticated downstream WebSocket traffic directly to CPA without a separate alias-rewrite sidecar.
...
`model-catalog-sidecar` (nginx-unprivileged, read-only, non-root) is a narrow catalog bridge: `GET /v1/models` forwards to `apisix-models` with no credential attached (the codex-models route requires none; the enricher holds its own CPA key), `GET /models-table` proxies to the Go server-rendered diagnostic page, and `POST /models-table/refresh` performs an explicit full-chain diagnostic refresh. The table has eleven columns, including separate `input_modalities` and `output_modalities`, and adjacent independent `max_tokens(abandon)`, `max_completion_tokens`, `max_output_tokens` columns. `abandon` marks only the deprecated Chat Completions request parameter, not an invalid catalog property. The complete table and refresh form arrive as HTML with no page JavaScript or browser JSON request. A successful forced refresh bypasses the enricher's CPA/AISIX, channel-inventory, metadata-source, and projection caches; concurrent submissions share one bounded collection. If that collection fails, the page keeps the last successful in-memory table and shows a warning with both timestamps; without a last-good table it returns an error page. Column values do not establish request budgets or protocol support. Missing fields display `未知`; explicit `null`, `false`, `0`, `""`, arrays and objects are rendered as text without fallback or HTML execution. Everything else gets 444. The sidecar joins both `apisix-catalog-source` (front side) and `apisix-catalog-target` (apisix-models side) /29 nets, same one-way shape as the socat relays. Both models-table paths are published only on Tailscale and localhost via `models-table-host-netns` + `models-table-relay` (`<tailscale-ip>:9083` / `127.0.0.1:9083`) — containers attached solely to `internal` networks cannot publish ports directly, so the netns-owner + socat pattern carries the exposure.

Ops notes:

- Unparameterized `/v1/models` on the **front** door keeps going straight to Sub2API (route `ai-api-models-get`), but shares the same global one-request admission slot as versioned catalog requests; on **apisix-models** it goes directly to CPA, as does an empty `client_version`.
...
- Troubleshoot catalog metadata via `http://127.0.0.1:9083/models-table` (or the Tailscale bind): missing capability cells show `未知`, while explicit nulls show `null`; enricher WARN logs name the failing channel `prefix`. The page's `POST /models-table/refresh` button bypasses the enricher's normal five-minute snapshot/read/projection caches, but it does **not** run or control the independent `cpa-model-sync` service, whose production interval is ten minutes. CPA must already have loaded a provider before the refresh can observe it. `proxy_max_temp_file_size 0` prevents concurrent large responses from exhausting the sidecar's small temporary filesystem.
- The front intersection validates identities and keeps selected records' original JSON fragments, avoiding Lua floating-point re-encoding of unknown numbers. Duplicate routing keys/collections fail closed. All external `/v1/models` and `/models` requests share one APISIX-wide admission slot across workers and callers; excess requests wait briefly, then receive bounded `503` instead of running concurrently. Entitlement-first ordering, final-entity ETags and 64 MiB limits remain in place; caching is confined to Go's upstream reads.
- Outbound pool slots remain held until response bodies close. Native/API-call, management inventory and bulk-source responses keep their respective 64/8/16 MiB limits; exceeding a limit is an error, not silent truncation.
