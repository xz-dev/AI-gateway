...
- `models.dev/api.json` outranks `models.json`, independently of fetch completion order. Adapters map only declared names, modalities and limits: an input limit is not total context, and a request parameter's default is not an output maximum. There is no capability pre-clear, model-name/template exception, or invented display name/text/reasoning/limit default. Each model layer aligns declared `max_output_tokens` and `max_tokens` (non-null canonical value wins, including zero).
...
- The gateway keeps the complete upstream inventory, including non-chat models and unknown capability fields. Nulls remain observable via `/models-table`. The separate [`openai-api-pi-extension`](https://github.com/xz-dev/openai-api-pi-extension) filters non-chat entries for Pi, retains image-input text models, and uses a 128,000 context fallback when a valid chat entry lacks a context limit. Output limits use the declared value, otherwise the resolved context limit. This fallback is client policy, not verified model capability.

The enricher keeps **no** cross-request caches. Concurrent builds for the same `client_version` share one in-flight result; different versions remain isolated. Response caching belongs to APISIX: key = `$uri` + `$arg_client_version`, fresh TTL 120s, HTTP 200 only. If an expired entry still exists, network errors, timeouts, HTTP 502/503/504, or an in-progress update may serve stale. A returned HTTP 500 is not masked by stale. The 120s fresh TTL is **not** an outage-time maximum age; cache lives in container `/tmp` and recreation starts cold. Without a usable cache, upstream failure propagates. The startup wrapper validates the generated APISIX location before enabling these cache settings. The client-facing CPA key comes from `CPA_CLIENT_KEY` (defaults to `CPA_API_KEY`).
