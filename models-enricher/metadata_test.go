...
func TestAdapterRecordsReachManifest(t *testing.T) {
	cases := []struct {
		...
		{"openai", `{"secret_envelope":"excluded","data":[{"id":"vendor/m:tag","name":"Declared","type":"image","context_length":0,"max_input_tokens":0,"max_output_tokens":0,"max_tokens":7,"architecture":{"input_modalities":["text"],"output_modalities":["image"]},"vendor":{"sequence":9007199254740993,"values":[null,false]},"supported_parameters":["reasoning"]}]}`,
			`{"slug":"c/vendor/m:tag","id":"c/vendor/m:tag","name":"Declared","display_name":"Declared","type":"image","context_length":0,"context_window":0,"max_input_tokens":0,"max_output_tokens":0,"max_tokens":0,"architecture":{"input_modalities":["text"],"output_modalities":["image"]},"input_modalities":["text"],"output_modalities":["image"],"vendor":{"sequence":9007199254740993,"values":[null,false]},"supported_parameters":["reasoning"]}`, parseOpenAI},
		...
		{"gemini", `{"secret_envelope":"excluded","models":[{"name":"models/g","displayName":"Gemini","inputTokenLimit":0,"outputTokenLimit":42,"supportedGenerationMethods":["generateContent","embedContent"],"output_modalities":["text"]}]}`,
			`{"slug":"c/g","name":"models/g","displayName":"Gemini","display_name":"Gemini","inputTokenLimit":0,"max_input_tokens":0,"outputTokenLimit":42,"max_output_tokens":42,"max_tokens":42,"supportedGenerationMethods":["generateContent","embedContent"],"output_modalities":["text"]}`, parseGemini},
	}
...
func TestSourceRecordsReachManifest(t *testing.T) {
	cases := []struct {
		...
		{"models.dev", "models.dev/p", `{"p":{"secret_provider_config":"excluded","models":{"dbid":{"id":"dbid","name":"Sourced","limit":{"context":0,"input":0,"output":0},"modalities":{"input":[],"output":["audio"]},"reasoning":true,"vendor_source":{"public":true},"sequence":9007199254740993}}}}`,
			`{"slug":"c/public","id":"c/public","name":"Sourced","display_name":"Sourced","limit":{"context":0,"input":0,"output":0},"context_window":0,"max_input_tokens":0,"max_output_tokens":0,"max_tokens":0,"modalities":{"input":[],"output":["audio"]},"input_modalities":[],"output_modalities":["audio"],"reasoning":true,"vendor_source":{"public":true},"sequence":9007199254740993}`,
		...
		{"modelparams explicit zero maximum", "modelparams.dev/p/api_key", `{"models":[{"model":"dbid","provider":"p","params":[{"path":"max_tokens","default":64,"range":{"max":0}}]}]}`,
			`{"slug":"c/public","model":"dbid","provider":"p","params":[{"path":"max_tokens","default":64,"range":{"max":0}}],"max_output_tokens":0,"max_tokens":0}`,
			func(tables *SourceTables, body []byte) { tables.mpK, tables.mpS = indexModelparams(body, testLog()) }},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
...
func TestNativeNumbersAndDeclaredLimitsSurvive(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/v1/models" || r.URL.Query().Get("client_version") != "fixture" {
	...
	base, err := cpa.NativeManifest(t.Context(), "fixture")
	if err != nil {
		t.Fatal(err)
	}
	tables := emptySourceTables()
	...
	assertMetadataJSON(t, manifest.Models, `[{"slug":"oauth/unlisted","context_window":272000,"max_tokens":128000,"max_output_tokens":128000,"vendor":{"sequence":9007199254740993}}]`)
}
...
`))
	...
	assertMetadataJSON(t, manifest.Models[1], `{"slug":"alias","max_tokens":0,"max_output_tokens":0,"display_name":"","input_modalities":[],"vendor":{"zero":0,"enabled":false,"sequence":9007199254740993,"keep":{"child":true}}}`)
}
...
func TestSharedSourceMetadataIsIsolated(t *testing.T) {
	...
	for i := 0; i < 8; i++ {
		t.Run(fmt.Sprint(i), func(t *testing.T) {
			...
			if a["max_tokens"] != 0 || a["max_output_tokens"] != 0 || b["max_tokens"] != 200 {
...
func TestOllamaPublicRecordAndSharedLookup(t *testing.T) {
	...
	hits := fetchOllamaForChannel(t.Context(), cpa, cfg, channel, models, testLog())
	manifest := mergeManifest(nil, []channelModels{{Channel: channel, Models: models}}, cfg, emptySourceTables(), map[string]map[string]sourceHit{"c": hits})
...
func TestSourceNullKeepsNativeAndChannelMetadata(t *testing.T) {
	...
	assertMetadataJSON(t, manifest.Models, `[{"slug":"c/m","id":"c/m","context_window":272000,"max_input_tokens":12345,"max_output_tokens":0,"max_tokens":0,"vendor":{"base":true,"channel":true}}]`)
}
...
func TestReferencesReadFixedSnapshot(t *testing.T) {
	...
	for _, reverse := range []bool{false, true} {
		...
		assertMetadataJSON(t, by["c/A"], `{"slug":"c/A","id":"c/A","max_tokens":40,"max_output_tokens":40,"input_modalities":["text","image"],"output_modalities":["text"],"vendor":{"a":true,"b":true,"overrideB":true,"target":false}}`)
		assertMetadataJSON(t, by["c/B"], `{"slug":"c/B","id":"c/B","max_tokens":30,"max_output_tokens":30,"input_modalities":[],"output_modalities":["text"],"vendor":{"b":true,"c":true,"overrideB":true}}`)
...
func TestStaticGenericInheritance(t *testing.T) {
	...
	for _, model := range manifest.Models {
		...
		assertMetadataJSON(t, model, fmt.Sprintf(`{"slug":%q,"id":%q,"max_tokens":0,"max_output_tokens":0,"input_modalities":[],"output_modalities":["image"],"vendor":{"existing":true,"lower":true,"pool":true,"final":true}}`, slug, slug))
	}
}
...
func TestOverrideFormsShareLayerSemantics(t *testing.T) {
	for _, flat := range []bool{false, true} {
		...
		assertMetadataJSON(t, manifest.Models, `[{"slug":"c/m","context_window":100,"max_tokens":0,"max_output_tokens":0,"vendor":{"keep":true,"enabled":false,"empty":""},"input_modalities":[]}]`)
	}
	...
	assertMetadataJSON(t, combined.modelOverrides("m"), `{"max_tokens":7,"max_output_tokens":7,"display_name":"Legacy","vendor":{"a":1,"b":0}}`)
}
...
func TestSourceFetchOrderDoesNotChangeMetadata(t *testing.T) {
	for _, apiFirst := range []bool{false, true} {
		t.Run(fmt.Sprint(apiFirst), func(t *testing.T) {
			...
			tables := fetchSources(t.Context(), newHTTPPool(3, time.Second), testLog())
			cfg := &Config{Channels: map[string]ChannelConfig{"c": {SourcePriority: []string{"modelparams.dev/p/api_key", "models.dev/p"}}}}
			manifest := mergeManifest(nil, []channelModels{{Channel: Channel{Prefix: "c"}, Models: []ParsedModel{{ID: "m"}}}}, cfg, tables, nil)
			model := manifest.Models[0]
			if toInt(model["context_window"]) != 111 || toInt(model["max_tokens"]) != 200 || toInt(model["max_output_tokens"]) != 200 {
