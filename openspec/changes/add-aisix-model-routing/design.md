...
AISIX resources contain one CPA provider key. The 25 concrete model resources reference that key, and each direct `display_name` preserves its exact CPA `model_name`, including `/`; the router does not invent a hyphenated second identity. The 12 client-visible bare-ID resources comprise nine ordered failover routes plus three round-robin routes. `grok-4.6` balances its two equal-weight targets. Because AISIX 1.2.0 resolves only one routing layer, `memory-fast` and `memory-reflect` retain expanded Grok direct targets; round-robin plus priority tiers preserves their outer fallback order while balancing only the same-tier Grok targets. `memory-consolidation` remains an independent two-target failover route. A concrete target reused by more than one logical route remains one direct resource, so its cooldown state is shared correctly across those routes while remaining independent from other targets.

Concrete cooldown is enabled with a 60-second default, a 300-second clamp, and `honor_retry_after: true`. With no per-model trigger override, AISIX cools a target on HTTP 401, 408, 429, 500, 502, 503, or 504 and on timeout/transport failures. This is a short target-level exclusion, not proof that provider quota has recovered; expiry only makes the target eligible for another attempt. Logical routes use one bounded retry with `retry_on_429: true`; Sub2API accounts use bounded retry and boolean pool mode so retries are not accidentally multiplied by string coercion.
...
For each `GET /status`, the renderer authenticates internally to AISIX's read-only model listing and runtime model-status endpoints. It parses only model identity, routing target order/strategy/budget, direct-target combo membership, and runtime exclusion/cooldown fields, then emits escaped HTML. It reads the existing AISIX bootstrap config through a read-only mount for the admin credential, avoiding a duplicate secret lifecycle. It never returns or logs the credential, original response body, provider-key reference, upstream URL, or request content.

The external listener accepts only `GET` and `HEAD` for `/status`; Admin, Scalar, playground, metrics, inference, and unknown paths are not proxied. The page has embedded CSS, no JavaScript or external assets, a 10-second automatic refresh, `no-store`, and a restrictive content-security policy. Empty explanatory sections and prose that merely repeats an `eligible` state are omitted. Errors produce a generic server-rendered unavailable page without backend detail.
