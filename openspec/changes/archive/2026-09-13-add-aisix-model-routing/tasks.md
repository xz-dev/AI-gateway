...
- [x] 5.1 Implement the minimal independently versioned server-rendered status service over fixed AISIX Admin API paths; verify focused HTTP tests cover configured target order, deterministic failover candidate, sorted direct combo memberships, concise runtime details, 10-second refresh, cooldown/unavailable details, HTML escaping, generic upstream failure, no JavaScript, no secret output, and rejection of every non-status path.
...
- [x] 5.6 Update operator documentation and redacted production evidence with final image IDs, topology, status 0.1.1 page semantics, exact CPA direct names, tiered Grok routing, accepted lack of authoritative last-served history, and maintenance access; verify strict OpenSpec validation and repository validators pass without exposing credentials or overwriting unrelated production changes.
