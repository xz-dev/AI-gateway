...
cleanup() {
  ...
  docker network rm "$internal_network" "$host_network" >/dev/null 2>&1 || true
  rm -rf -- "$tmpdir"
}
trap cleanup EXIT
...
docker image inspect "$guard_image" >/dev/null 2>&1 || docker pull "$guard_image" >/dev/null
docker image inspect "$python_image" >/dev/null 2>&1 || docker pull "$python_image" >/dev/null
port=$(python3 - <<'PY'
import socket
...
[ -z "$(docker exec "$owner" ip -4 route show default)" ] || { echo 'namespace owner retained an IPv4 default route' >&2; exit 1; }
[ "$(docker exec "$owner" sh -c "awk '/^CapEff:/ {print \$2}' /proc/1/status")" = 0000000000000000 ] || {
  echo 'namespace owner did not drop effective capabilities' >&2
  exit 1
}
[ -z "$(docker exec "$app" sh -c "awk 'NR > 1 && \$2 == \"00000000\" && \$8 == \"00000000\" {print}' /proc/net/route")" ] || {
  echo 'application started before namespace hardening' >&2
  exit 1
}
internal=$(docker run --rm --network "$internal_network" "$python_image" python3 -c \
...
if timeout 5 docker exec "$app" python3 -c 'import socket; socket.create_connection(("1.1.1.1", 443), 3)' >/dev/null 2>&1; then
