...
def serve(kind, sock):
 while True:
  if kind=="tcp":
   c,_=sock.accept(); threading.Thread(target=lambda: (c.sendall(c.recv(4096)),c.close()),daemon=True).start()
  else:
...
echo 'socat_tcp=valid socat_udp=valid reverse_initiation=blocked privileges=dropped pairwise_networks=valid'
